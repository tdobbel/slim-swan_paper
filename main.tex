...
% SUMMARY
Validation against field measurements show that the coupled SLIM+SWAN model correctly reproduce hydrodynamics and wave dynamics during hurricane Irma. Such good agreement is obtained using accurate forcings and adequate wave parameterizations. Furthermore, comparing coupled and uncoupled model runs suggests that neglecting wave radiation stress gradient induces differences of up to 1 m/s in modeled current velocities. Radiation stress gradient during the hurricane was especially large over the shelf break, where waves are strongly dissipated by the protective coral barrier and can lead to variations of more than 10 km in modelled trajectories. These differences in modelled trajectories were significantly larger and required more time to stabilize on the outer shelf. The impact of the Stokes drift dominates the effects of wave-current interactions through the radiation stress gradient, except at  the moment of the passage of the hurricane, when both contributions are similar. Finally, neglecting wave-current interactions when computing Stokes drift lead to variations of up to 2 km in modelled trajectories on the passage of the hurricane.
...
Despite its good agreement with observations, our model lacks in the representation of the wind-wave interactions. In particular, it does not consider the momentum loss due to the action of surface waves when representing momentum flux from the atmosphere to the ocean, leading to overestimations under hurricane conditions. Our model could therefore be further improved by using wave-dissipative stress instead of the full wind stress as the momentum flux from the atmosphere to the ocean. Moreover, a more thorough calibration of the wave model parameters should improve our model results on the eastern shelf of Florida. 

Wave coupling needs to be taken into account during heavy-wind events but not necessarily in milder conditions. While the wave-current interaction plays an important role and can lead to differences of up to \textcolor{red}{[insert value]}, the Stokes drift is about \textcolor{red}{[insert value]} more intense and should thus be considered in priority
